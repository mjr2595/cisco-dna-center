...
def get_token(
    ...
    response = requests.request(
        "POST", url, auth=HTTPBasicAuth(username, password), verify=False
    )
...
def get_commands(
    token, host=env_lab.DNA_CENTER["host"], port=env_lab.DNA_CENTER["port"]
):
    url = "https://{}:{}/api/v1/network-device-poller/cli/legit-reads".format(
        host, port
    )
